...
- **反应强度匹配**：角色对事件的反应是否与事件严重性匹配。普通人发现身体失去知觉 → 应有恐慌/自我欺骗/求助，不能只是"站在那里"；角色目睹死亡 → 应有生理反应（恶心、腿软、发抖），不能平静离开。severity: medium, category: character
...
若章节文件包含改写标记（`rewrite_version` 字段或 `.webnovel/archive/` 中有历史版本），额外检查：

| 检查项 | 严重度 | 阻断 |
|--------|--------|------|
| 伏笔连接断裂（open_loops 中引用本章的条目在改写后消失） | critical | true |
| 设定引用不一致（角色境界、地点、物品与 index.db 矛盾） | critical | true |
| 承接钩子消失（上章末尾的承诺在改写后未被回应） | high | true |
| 章节钩子消失（本章末尾的悬念在改写后消失） | high | true |
...
1. 加载本章涉及角色的约束
2. 检查正文中角色行为是否违反约束
3. 违反 hard constraint → severity: critical, blocking: true
4. 违反 soft constraint → severity: medium, blocking: false
5. evolving constraint 的变化需要有剧情铺垫 → 无铺垫则 severity: high

输出到 issues 数组，category: `character_constraint`。
