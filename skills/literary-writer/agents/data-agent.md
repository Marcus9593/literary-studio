...
在完成提取后，对比新提取的实体/设定与现有 `MASTER_SETTING.json`：

1. 加载 `MASTER_SETTING.json`（从 `.story-system/` 或项目根目录）
2. 对比 `extraction_result` 中的：
   - `entity_deltas`：新实体是否与现有设定冲突
   - `state_deltas`：状态变化是否违反设定规则
   - `accepted_events` 中的 `world_rule_revealed`：新规则是否与现有规则矛盾
3. 为每个检测到的冲突生成：
   - `conflict_type`: `entity | setting | rule | timeline`
   - `severity`: `critical | high | medium | low`
   - `confidence`: 0.0-1.0
   - `description`: 冲突描述
   - `existing_rule`: 现有设定中的相关规则
   - `new_claim`: 新章节中的声称
4. 将冲突列表写入 `extraction_result.setting_conflicts` 数组

若存在 `severity=critical` 且 `confidence ≥ 0.8` 的冲突，在摘要中标注"设定冲突"。
