...
- 章末禁止安全着陆，留未解决的问题
- 展示后不解释
- 场景转换必须交代空间移动——角色从 A 到 B 需要有物理路径（走楼梯、骑车、拐弯），不能凭空消失又凭空出现
- 角色反应强度匹配事件严重性——面对异常/危险/冲击事件时，反应不能太淡（如发现身体失去知觉应有恐慌或自我欺骗，不能只是"站在那里"）

## 3. 执行流程

### 改写模式（当 mode=rewrite 时）

额外输入：
- `original_chapter_file`：已有章节文件路径
- `rewrite_instructions`：改写指令（用户要求或审查报告）
- `must_preserve`：必须保留的元素列表（foreshadowing_links, setting_consistency, chapter_hooks）

执行额外步骤：
1. `Read` 已有章节正文
2. 从 `rewrite_instructions` 中提取改写重点
3. 查询本章涉及的伏笔连接（`open_loops` 中引用本章的条目）
4. 查询本章的设定依赖（chapter_meta 中的 setting references）

在标准五段任务书基础上，额外输出两段：

### 6. 必须保留的元素
- 伏笔连接：[列出本章涉及的 open_loops]
- 设定引用：[列出本章涉及的实体状态]
- 承接钩子：[列出上章承诺和本章钩子]
...
检查是否存在 `设定集/角色行为约束.md`：
- 若存在，读取本章涉及角色的约束
- 将约束转化为第 3 段"这章的人物"中的行为边界提示
- 示例：若角色有 hard constraint "不杀无辜"，在任务书中提醒："此角色不会对无辜者动手，即使形势逼迫。"
...
当 load-context 返回 `layered=true` 或当前章节 ≥ 100 时，使用 3 层上下文：

**Layer 1: Global（全局层）**
- 包含：压缩的总纲、核心设定、角色关系图、力量体系摘要
- 用途：确保不违反全局约束
- 大小限制：< 5000 字符

**Layer 2: Recent（近期层）**
- 包含：最近 N 章摘要（N=10 for 100+章, N=5 for 50+章）、近期事件、活跃伏笔、当前剧情弧
- 用途：确保剧情连贯性
- 大小限制：< 8000 字符
...
组装顺序：Current → Recent → Global（按重要性递减）。若总 token 超限，优先裁剪 Global 层。
...
5. 若存在 `设定集/角色行为约束.md`，读取本章涉及角色的约束。
