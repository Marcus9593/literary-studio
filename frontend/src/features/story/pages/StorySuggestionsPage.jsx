...
export default function StorySuggestionsPage() {
  ...
  const goToWorkspaceWithPlan = (plan, userMessage) => {
    navigate(`/projects/${projectId}`, {
      state: {
        pendingPlanExecution: {
          ...plan,
          user_message: userMessage || plan.user_request,
        },
      },
    })
  }

  const onStartAction = async (item) => {
    if (!item?.id) return
    setBusyActionId(item.id)
    try {
      const plan = await createActionPlan(projectId, item.id)
      showToast('正在工作台执行改稿…', 'success')
      goToWorkspaceWithPlan(plan, item.title)
    } catch (e) {
      showToast(e.message, 'error')
    } finally {
  ...
  const onStartTask = async (taskId) => {
    ...
    try {
      ...
      goToWorkspaceWithPlan(prep.plan, prep.user_message)
    } catch (e) {
      showToast(e.message, 'error')
    } finally {
      setBusyTaskId(null)
  ...
  return (
    <StoryOsPage
      className="story-suggestions-page"
      projectTitle={project?.title}
      loading={loading && !todayData}
      loadingLabel="加载今日创作…"
    >
