...
import { withProjectSessionLock } from '../lib/project-session-lock.js';
...
export function updateSessionFields(projectId, sessionId, patch = {}) {
  return withProjectSessionLock(projectId, () => {
...
export function focusChapterSession(projectId, filename, chapterTitle = '') {
  return withProjectSessionLock(projectId, async () => {
    ...
    const session = await createSession(projectId, title, { bound_filename: safe });
...
export function focusProjectSession(projectId) {
  return withProjectSessionLock(projectId, async () => {
    ...
    if (!entry) {
      await createSession(projectId, '全书讨论', { bound_filename: SESSION_SCOPE_PROJECT });
    ...
    if (entry) await setActiveSession(projectId, entry.id);
    index = listSessions(projectId);
