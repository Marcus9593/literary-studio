...
| **Docker 镜像** | 官方镜像 `literarycraft/studio`，单容器集成前后端与 Python 文档转换 |
...
| 标签 | 说明 |
|------|------|
| `latest` | 完整版（Node 22 + Python，支持 DOCX/PDF 转换） |
| `2.6.0` | 与当前发布版本对应 |
| `slim` | 精简版（仅 Node，无文档转换） |
...
docker run -d \
  ...
  -e STUDIO_JWT_SECRET=your_random_secret_32_chars_min \
  -e STUDIO_ADMIN_PASSWORD=your_strong_password \
  -e STUDIO_ALLOW_REGISTER=0 \
  literarycraft/studio:latest
```

浏览器访问 **http://127.0.0.1:8765**，使用 `.env` 中配置的管理员账号登录。

### 方式二：Docker Compose（推荐本地 / 自建）
...
| 项目 | 值 |
|------|-----|
| 镜像仓库 | [hub.docker.com/r/literarycraft/studio](https://hub.docker.com/r/literarycraft/studio) |
| 容器端口 | `8765` |
| 数据目录 | `/app/data`（请挂载 volume，升级镜像不丢数据） |
| 环境变量模板 | `.env.docker.example` |

更详细的构建、多架构发布与故障排查见 [`deploy/docker/README.md`](deploy/docker/README.md)。
...
│   └── build.mjs             # 前端构建
├── data/                     # 运行时数据（不提交 Git）
├── Dockerfile                # 镜像构建（literarycraft/studio）
├── docker-compose.yml        # Compose 编排
├── docker-entrypoint.sh      # 容器启动入口（数据目录权限）
...
常见问题（端口占用、页面空白、Node 版本、AI 无响应、文档导入、Docker 部署等）见 **[`deploy/README.md` — 故障排查](deploy/README.md#故障排查)**。
...
| Python | 3.9+ | Optional |
| Docker | Docker 20+ & Compose | Optional (recommended) |

### Quick Start

**Option A — npm (from source):**
...
**Option B — Docker (no Node/Python install required):**
...
Open **http://127.0.0.1:8765** — login with `admin` / `admin123` (npm) or your configured password (Docker).

> ⚠️ Change the default password and JWT secret before any production use!

Official image: [`literarycraft/studio`](https://hub.docker.com/r/literarycraft/studio) — see [`deploy/docker/README.md`](deploy/docker/README.md).
...
Cross-platform installation, production deployment (systemd / launchd / Windows service / Docker), environment variables, and troubleshooting: **[`deploy/README.md`](deploy/README.md)**.
...
├── deploy/            # Deployment configs (incl. deploy/docker/)
├── Dockerfile         # Image: literarycraft/studio
├── docker-compose.yml
